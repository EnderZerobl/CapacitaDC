...
export default function GamesPage() {
  ...
  const canAuthor = user?.type === "admin" || user?.type === "organizador"
  useEffect(() => {
    if (isLoading) return
    if (!user) router.replace("/login")
    else if (!canAuthor) router.replace(user.type === "trainee" ? "/trainees" : "/membros")
  ...
  return <main className="mx-auto min-h-screen max-w-6xl space-y-7 px-4 py-8 sm:px-6">
    ...
    <GameLibrary isOrganizer={user?.type === "organizador"} />
  </main>
}
